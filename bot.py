...
# 可以指定額外的管理員用戶 ID
ADMIN_USER_IDS = []  # 例如：[123456789, 987654321]
...
async def show_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if transactions:
        ...
        text += f"累计应下发：{total_income_all:,.1f} | {total_income_all / exchange_rate:.2f} u\n"
        text += f"累计已下发：{total_expense_all:.0f} | {total_expense_all / exchange_rate:.2f} u\n"
        text += f"累计未下发：{cumulative_balance:,.1f} | {cumulative_balance_u:.2f} u"
        await update.message.reply_text(text, parse_mode='Markdown')
    else:
        text = f"""📊 *今日無交易記錄*
...
累计应下发：{total_income_all:,.1f} | {total_income_all / exchange_rate:.2f} u
累计已下发：{total_expense_all:.0f} | {total_expense_all / exchange_rate:.2f} u
累计未下发：{cumulative_balance:,.1f} | {cumulative_balance_u:.2f} u"""
        await update.message.reply_text(text, parse_mode='Markdown')
